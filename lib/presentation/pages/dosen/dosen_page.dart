...
class DosenPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<DosenDashboardViewModel>(
...
class _DosenPageView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Ringkasan: HP tetap 2 kolom, desktop bisa 3
    final ringkasanCount = width >= 900 ? 3 : 2;

    // Rasio grid biar aman dari overflow (samain Admin)
    final ringkasanRatio = width >= 1200 ? 2.4 : (width >= 900 ? 2.1 : 1.65);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Notifikasi',
            onPressed: () => vm.snack(context, 'Notifikasi belum tersedia'),
            icon: const Icon(
          ...
          IconButton(
            tooltip: 'Pengaturan',
            onPressed: () => vm.snack(context, 'Pengaturan belum tersedia'),
                ...
                slivers: [
                  SliverToBoxAdapter(
                    child: Padding(
                      ...
                      child: const _HeroBannerDosen(primary: _primary),
                    ),
                  ),
                      ...
                      delegate: SliverChildListDelegate([
                        _MenuTileModern(
                          ...
                          onTap: () => Navigator.push(
                            context,
                            _fadeRoute(const JadwalDosenPage()),
                          ),
                        ),
                        _MenuTileModern(
                          title: 'Absensi',
...
class _HeroBannerDosen extends StatelessWidget {
  final Color primary;
  const _HeroBannerDosen({required this.primary});
...
/// ✅ versi FIX (sama seperti Admin): auto compact + auto narrow biar gak overflow
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, c) {
        ...
        final narrow = c.maxWidth < 190;

        final titleSize = compact ? 12.5 : 13.0;
        final valueSize = compact ? 20.0 : 22.0;
        final chipSize = compact ? 10.5 : 11.5;

        final gapTop = compact ? 4.0 : 8.0;
        final gapMid = compact ? 6.0 : 10.0;
        ...
        Widget chip() {
          return Container(
            ...
            decoration: BoxDecoration(
              color: const Color(0xFF0E2E72).withOpacity(0.08),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    // bawah disembunyikan saat compact biar gak overflow
                    if (!compact) ...[
...
class _MenuTileModern extends StatelessWidget {
  final String title;
  final String subtitle;
  final IconData icon;
  final Color primary;
